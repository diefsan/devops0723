    ...
    stages {
        ...
        stage('Build Images') {
            steps {
                script {
                    // Nome das imagens
                    def imageNameWeb = 'sandrodief/web'
                    def imageNameDB = 'sandrodief/db'
                    def imageNameNginx = 'sandrodief/nginx'

                    // Executa o build das imagens com os respectivos Dockerfiles
                    docker.withRegistry('https://registry.hub.docker.com', 'dockerhubsandrodief') {
